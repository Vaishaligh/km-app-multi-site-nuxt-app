  ...
  <div class="container-fluid cart-container section-main-container wishlist">
    <div class="row">
      <div class="col-md-12 product-section-main">
        ...
        <div  v-if="wishListData">
           <div v-html="wishData" v-show="displayWish"></div>
            <Shimmer v-show="displayWish==false"/>    
        </div>
        <div v-else-if="this.wishlistProduct.length">
                      ...
                      >
                        <p class="">
                          ...
                          <span
                            class="old-price"
                          >
                          <i class="kartmax_currency"></i>{{article.price}}
                          </span>
...
import Shimmer from "../../components/custom/PlpShimmer";
...
var app = new Vue({
  methods: {
    kmAddRemoveWishlistAction: function(param,g_id,sub_group_id) {
        that.remove_w(param,g_id,sub_group_id)
    },
  }
})
var kmAddRemoveWishlistAction = function(param,g_id,sub_id) {
  app.$options.methods.kmAddRemoveWishlistAction(param,g_id,sub_id);
};
export default {
  name: "wishlist",
  mixins:[modifyMixin],
  ...
  methods: {
    ...
    async getWishData(){
       that = this
      //  this.wishData = ''
        await axios.get(this.$store.state.list.editor_url+'/data/wishlist').then(wish=>{
            this.wishData =  wish.data.data;
            this.wishListData = true
            // this.replace = true
            this.script = wish.data.scripts
            // this.replaceCode()
            this.displayWish = true
          
        }).catch(err=>{
          this.replace = false
          console.log("err in wishlist page",err
        )})
    },
      getClass(){
      return {
        'v-hide-class': this.displayWish,
        'v-show-class': !this.displayWish
        }
    },
      ...
      axios
        ...
        .then(async (response) => {
          if (response.data.success == true) {
            ...
            if(this.wishlistProduct.length > 0 && this.replace == true)
            await this.replaceCode('delete')
            let wishList = document.getElementById("wishlist-count");
            if (response.data.data.total_row) {
              wishList.innerHTML = response.data.data.total_row;
            } else {

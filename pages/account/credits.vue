      ...
      <div class="row">
        ...
        <div class="col-md-9">
          <div class="row">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      ...
                      >
                        ...
                        <p
                          style="font-size: 20px; margin: 0; font-weight: bold" class="text-center"
                        >
                       {{(total)}}
                        </p>
                        <div class="col-md-12 d-flex">
                          <div class="col-md-6 text-start">Total Value of points
                            <br/>
                            <span v-if="total>0" style="color:gray"><i class="kartmax_currency"></i>{{(total / creditsPerRupee)  }}</span>
                            <span v-else style="color:gray">0</span>
                          </div>
                          <div class="col-md-6 text-end">Points per Rupee
                          <br/>
                          <span style="color:gray">{{creditsPerRupee}}</span></div>
                        </div>
            ...
            <div class="col-md-12" v-if="transactions.length > 0">
              <div class="row mt-4">
                <div class="col-md-12">
                  <div class="content-box">
                    <div class="row">
                      <table class="table order-table store">
                        ...
                        <tbody>
                          <tr v-for="(log, index) in transactions" :key="index">
                            ...
                            <td
                              class="des"
                              v-if="log.is_credit == 0 && log.quote && log.quote.order_increment_id"
                            >
                                Debited for {{log.credited_for}} {{ log.quote.order_increment_id }}
                            </td>
                            <td
                              class="des"
                              v-else-if="
                                log.is_credit == 1 && log.quote && log.quote.order_increment_id
                              "
                            >
                              Credited for {{log.credited_for}}
                              {{ log.quote.order_increment_id }}
                            </td>
                            <td v-else-if="log.credited_for">
                              {{ log.credited_for }}
                            </td>
                            <td class="amount">
                              {{ log.is_credit == 0 ? "-" : "+" }}
  ...
  methods: {
    ...
    storeLogs() {
      ...
      axios
        ...
        .then((response) => {
          this.total = response.data.data.creditDetails.totalAvailablePoints;
          this.creditsPerRupee = response.data.data.creditDetails.creditsPerRupee;

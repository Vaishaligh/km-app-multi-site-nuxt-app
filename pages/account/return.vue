            ...
            <div class="col-md-12">
              <div class="content-box">
                <table class="table order-table return-ex-table responsive-table" v-if="order.length">
                  ...
                  <tbody >
                    <tr  v-for="(items,index) in order" :key="items">
                      <td class="text-start" >
                          <div>
                            ...
                            <nuxt-link style="text-decoration: none;" :to="{ path: '/product/' + items.url_key }">        <h6 class="item-name kartmax-text-f-14 my-1" v-text="items.name">
                                   </h6>    </nuxt-link>
                      ...
                      <td v-text="items.created_date" class="order-date kartmax-text-f-14"></td>
  ...
  export default {
    ...
    methods: {
       view_status(awbNumber){
        ...
        axios.post(this.$store.state.list.api_url+'/pim/tracking.php', form)
        .then(response => {
      ...
      items_chat(v,rid) {
        ...
        this.get_comments(rid);

      },
      close_commentbox(){
        this.chatid = 999;
      },
      get_comments: function (rid) {
        if (this.$store.state.list.customer_id != null && this.$store.state.list.customer_id != '') {
          ...
          var form = new FormData();
          form.append("return_id", rid);
          axios.post(this.$store.state.list.cart_api_url+'/v1' + '/customer/return-comment', form)
            .then(response => {
              if (response.data.success === true) {
                
                this.commentLoader = false;
                this.ordercomments = response.data.data;
                this.ordercomments = this.ordercomments.reverse();
                for (let i = 0; this.ordercomments.length; i++) {
                    if(this.ordercomments[i].User == 'Admin'){
                ...
                this.noComments = 'No Comments Found';
                this.commentLoader = false;
      ...
      addcomment: function (rid,comments) {

        var commentValid = /^\s*$/ ;
        if(comments.match(commentValid)){
          this.comment_error = 'Please write a comment.'
        } else {
          this.ordercomments = [];
        if (this.$store.state.list.customer_id != null && this.$store.state.list.customer_id != '') {
          this.$store.state.list.error_message = '';
      ...
      reorder(cart_id) {
        ...
        axios.post(this.$store.state.list.cart_api_url+'/v1' + '/order/re-order', form)
          .then(response => {
            if (response.data.success === true) {
      ...
      get_account: async function  () {
        if (this.$store.state.list.customer_id != null && this.$store.state.list.customer_id != '') {
          ...
          await axios.post(this.$store.state.list.cart_api_url+'/v1' + '/customer/returned-items', form)
            .then(response => {
              if (response.data.success === true) {
                this.$store.state.list.page_loader = false;
      ...
      request_return(order,item_id,itemqty,name,sku){
        ...
        if (this.$store.state.list.customer_id != null && this.$store.state.list.customer_id != '') {
          ...
          axios.post(this.$store.state.list.cart_api_url+'/v1' + '/customer/return-request', form)
            .then(response => {
              if (response.data.success === true) {
                this.$store.state.list.page_loader = false;
                this.select_o_n='';
    ...
    updated(){
      // jQuery('.r_item').click(function(){
      //    var trheight = jQuery(this).parents('tr').height();
      //    //window.scrollTo(0, jQuery(this).offset().top + trheight - 100);
      // });
    },
